...
export default function Rating ()
{
  ...
  const getData = async () => {
    try {
            const res = await axios.get(`http://localhost:8080/reviewbycourse/1`)
            if(res.data !== "")
            {
  ...
  const postData = async () => {
    ...
    const studentid=4;
    try {
            const res = await axios.post(`http://localhost:8080/addreview`, { fifth_rating , first_rating , fourth_rating , second_rating , third_rating, courseid , studentid})
            if(res.data !== "")
            {
